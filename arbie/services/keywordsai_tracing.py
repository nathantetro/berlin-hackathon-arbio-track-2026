"""Keywords AI tracing integration for OpenAI Agents SDK.

This module provides a custom trace processor that exports agent traces to Keywords AI.
Adapted from keywordsai-exporter-openai-agents package to work with openai-agents>=0.7.0.
"""
        ...
        if span_data.model_config:
            for param in [
                ...
                if param in span_data.model_config:
                    setattr(data, param, span_data.model_config[param])
...
def _guardrail_data_to_keywordsai_log(
    ...
    data.log_type = "guardrail"
    data.has_warnings = span_data.triggered
    if span_data.triggered:
        data.warnings_dict = data.warnings_dict or {}
        data.warnings_dict = {f"guardrail:{span_data.name}": "guardrail triggered"}


class KeywordsAISpanExporter(BackendSpanExporter):
    """Custom exporter for Keywords AI that handles all span types."""
    ...
    def _keywordsai_export(
        ...
        """Process span types and extract JSON serializable attributes."""
        if isinstance(item, Trace):
            ...
            try:
                ...
                logger.error(f"Error converting span data to KeywordsAI log: {e}")
                return None
        else:
            return None
...
class KeywordsAITraceProcessor(BatchTraceProcessor):
    """A processor that sends traces and spans to Keywords AI."""
